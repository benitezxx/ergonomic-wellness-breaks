...
# Iniciar captura de video (camara por defecto)
...
with mp_pose.Pose(min_detection_confidence=0.5, min_tracking_confidence=0.5) as pose:
    while cap.isOpened():
        ...
        try:
            ...
            # Obtener visibilidad de los puntos clave
            vis_cadera = landmarks[mp_pose.PoseLandmark.LEFT_HIP.value].visibility
            vis_rodilla = landmarks[mp_pose.PoseLandmark.LEFT_KNEE.value].visibility
            vis_tobillo = landmarks[mp_pose.PoseLandmark.LEFT_ANKLE.value].visibility
            
            # Solo si está más del 60% seguro de ver los tres puntos, calcula el angulo
            if vis_cadera > 0.6 and vis_rodilla > 0.6 and vis_tobillo > 0.6:
                ...
                # Aviso de cuerpo completo
                cv2.putText(image, 'CUERPO INCOMPLETO - ALEJATE', (100, 200), cv2.FONT_HERSHEY_SIMPLEX, 1, (0,0,255), 2, cv2.LINE_AA)
                
        except:
            pass # Si no detecta cuerpo, no hace nada y sigue
        
        # Mostrar el contador y estado en pantalla
